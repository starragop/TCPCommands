...
def install():
    with urllib.request.urlopen("https://raw.githubusercontent.com/starragop/TCPCommands/master/info") as url:
        filepage = url.read()
    ...
    if progressexists == True:
        with Bar('Downloading files...', max=(len(data))) as bar:
            for i in range(len(data)):
                ...
                print("Installing " + x)
                dat = urllib.request.urlopen("https://raw.githubusercontent.com/starragop/TCPCommands/master/" + x)
                file = open(x,"w")
